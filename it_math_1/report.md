...
Результаты и сравнения с теоретическими можно
увидеть [тут](https://docs.google.com/spreadsheets/d/1jxdAneSE_nq_82XzDoQFgnv49XIwvKHlEUJbJJ0U0bY/edit?usp=sharing#gid=676081853)
...
Результаты и сравнения с теоретическими можно
увидеть [тут](https://docs.google.com/spreadsheets/d/1jxdAneSE_nq_82XzDoQFgnv49XIwvKHlEUJbJJ0U0bY/edit?usp=sharing#gid=1201301379)

## О результатах

Как видно затраты на параллельность оправдывают себя только при больших N, когда в потоке совершается достаточно большое
количество операций. Так параллельность дает ощутимый прирост при N > 500, а существенный прирост при N > 1000, в то
время
как на N = 100, наоборот, параллельность негативно сказывается на производительности. 

Это сказывается и на алгоритме 11.6, в нем 
необходимо найти баланс между размером чанка и их количеством для достижения оптимальной производительности. Так при N = 498
и размере чанка 50 были показаны результаты хуже, чем при размере чанка = 100, т.к. поток выполнял куда меньшую вычислительную
задачу и, следовательно, прирост производительности относительно затрат был куда меньше.

Также еще можно заметить, что на большом количестве потоков, результаты иногда хуже, чем на малом.

### Гипотеза, почему на большом кол-ве потоков результаты хуже

Тесты на производительность на количестве потоков 4+ в большинстве случаев показывали производительность хуже, чем на 3
или 2 потоках.
Мое предположение заключается в том, что другие программы "едят" процессорное время части ядер, из-за этого какой-то
поток не
исполняется, а "висит" какое-то время. Так как для перехода на следующую итерацию требуется, чтобы все потоки
выполнились,
